...
def log(text, array=None):
     """ 打印文本信息 """
     log_folder = f"./log/log-{timestamp}"
     if not os.path.exists(log_folder):
         os.makedirs(log_folder)
     log_file = os.path.join(log_folder, 'train_text.txt')
...
def compute_backbone_shapes(config, image_shape):
    """ 计算backbone在给定尺寸下的输出特征图形状，深度、宽度和高度。
        Returns:[N, (depth, height, width)]. N是stages数
    """
    H, W, D = image_shape[:3]
    return np.array(
...
class FPN(nn.Module):
    """ 特征金字塔模块，将三个阶段的特征图C1,C2,C3转化为相同通道数，再卷积"""
...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=None):
    """ 生成候选框
        接收anchor分数，排序后选择一个子集作为proposals传递给第二阶段。
    ...
    # Box的得分，前景类置信度 [Batch, num_rois, 1]
    scores = inputs[0][:, 1]

    # Box 细化 [batch, num_rois, 6]
    deltas = inputs[1]
...
def RoI_Align(feature_map, pool_size, boxes):
    """ 实现3D RoI Align，将不同的候选框对齐到固定大小的输出
...
def pyramid_roi_align(inputs, pool_size, test_flag=False):
    """ 在特征金字塔上实现 ROI align
        ...
        - image_shape: [height, width, depth, channels].
        Inputs:
        - boxes: [batch, num_boxes, (z1, y1, x1, z2, y2, x2)] 在标准化坐标系中。
        - Feature maps: 来自金字塔不同层的特征图列表。[batch, channels, depth, height, width]
        Output:
    ...
    # 裁剪 boxes [batch, num_boxes, (y1, x1, z1, y2, x2, z2)]
    boxes = inputs[0]
...
def bbox_overlaps(boxes1, boxes2):
    ...
    """
    # 允许在没有循环的情况下比较每个boxes1和每个boxes2。
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # Positive ROIs 是指 GT box 的 IoU >= 0.7
    ...
    # 附加 negative roi，并用0填充不用于negative roi的 bbox的deltas和mask。
...
def detection_layer(config, rois, mrcnn_class, mrcnn_bbox, image_meta):
    """ 修正分类 proposal boxes和 bbox deltas，返回最终检测框。
        Returns:
...
class RPN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Anchor 得分. [batch, anchors per location * 2, depth, height, width].
        rpn_class_logits = self.conv_class(x)

        # 形状重塑 [batch, anchors, 2]
        rpn_class_logits = rpn_class_logits.permute(0, 2, 3, 4, 1)
        rpn_class_logits = rpn_class_logits.contiguous()
        rpn_class_logits = rpn_class_logits.view(x.size()[0], -1, 2)

        # Softmax 在最后一个维度上 前景或者背景.
        rpn_probs = self.softmax(rpn_class_logits)

        # 细化bbox. [batch, anchors per location * 6, D, H, W]
        # 其中 6 == delta [z, y, x, log(d), log(h), log(w)]
        rpn_bbox = self.conv_bbox(x)

        # 重塑形状 [batch, anchors, 6]
...
def build_rpn_targets(anchors, gt_boxes, config):
    """ 给定anchor和GT box，计算iou，识别positive anchor和delta，
    ...
    # 计算iou重叠 [num_anchors, num_gt_boxes]
    ...
    # 首先设置negative anchor。如果GT box与它们匹配，它们将被覆盖在下面。避开拥挤区域的box。
    anchor_iou_argmax = np.argmax(overlaps, axis=1)
    anchor_iou_max = overlaps[np.arange(overlaps.shape[0]), anchor_iou_argmax]
    rpn_match[anchor_iou_max < 0.3] = -1

    # 为每个GT box设置一个anchor(无论IoU值如何)。
    gt_iou_argmax = np.argmax(overlaps, axis=0)
    rpn_match[gt_iou_argmax] = 1

    # 将高重叠的anchor设置为positive.
...
class FasterRCNN(nn.Module):
    ...
    def build(self, config, test_flag=False):
        ...
        self.anchors = Variable(torch.from_numpy(utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                        config.RPN_ANCHOR_RATIOS,compute_backbone_shapes(config, config.IMAGE_SHAPE),
                        config.BACKBONE_STRIDES,config.RPN_ANCHOR_STRIDE)).float(),requires_grad=False)
    ...
    def detect(self, images):
        ...
        # 转换为numpy
            ...
            print("检测用时：", time.time() - start_time)
    ...
    def predict(self, inputs, mode):
        ...
        if mode == 'inference':
            ...
            return [rpn_class_logits, rpn_bbox,
                    ...
                    target_deltas, mrcnn_bbox, target_mask]
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs):
        ...
        if not os.path.exists("./weight/heart/" + str(start_datetime)):
            os.makedirs("./weight/heart/" + str(start_datetime))
        for epoch in range(self.epoch + 1, epochs + 1):
            ...
            for epoch in range(epoch % 5 == 0):
                    if val_loss < best_val_loss:
                        ...
                        folder_path = "./weight/heart/" + str(start_datetime)
    ...
    def train_epoch(self, datagenerator, optimizer, steps, angle, dataset):
        ...
        for inputs in datagenerator:
            ...
            images, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks = load_image_gt(image, mask, angle, dataset,
                                                                            self.config,self.anchors.cpu().numpy())
    ...
    def unmold_detections(self, detections, image_shape, window):
        ...
        print("unmold完成,用时：", time.time() - start_time)

        return boxes, np.arange(1, 8), scores
